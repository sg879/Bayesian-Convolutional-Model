...
def Neg_ELBO(Sigma_f, Ell, Sigma_n, Z_Fractions, V_Vector, L_Diag, L_ODiag, TRise,
  ...
  Likelihood = Likely(Pred, Velocity, Sigma_n, N).mean(axis = -2)

  KL = lax.psum(KL, axis_name="machs")
  Exp = jnp.sum(Likelihood)
                  
  return (KL - Exp)/(Num_Filt * 3 * (N + 1))
...
iopt_key = np.asarray([random.PRNGKey(1) for i in range(num_mach)]).astype(np.uint32)
...
for i in trange(coarse_steps):  
  sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, opt_subkey, opt_key, mad, vad, value = _CUpdate(i, sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, num_f, num_b, num_ind, num_filt, batch_size, k, n, indices, spikefft, yvel, ftime, opt_subkey, opt_key, mad, vad)
  sigma_n = sigma_n[0]
  elbo_history[step] = -value[0]
  step += 1

#%%
print(PNeg(sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, num_f, num_b, num_ind, num_filt, batch_size, k, n, indices, spikefft, yvel, ftime, opt_subkey))
# %%
...
for i in trange(mid_steps): 
  sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, opt_subkey, opt_key, mad, vad, value = _MUpdate(i, sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, num_f, num_b, num_ind, num_filt, batch_size, k, n, indices, spikefft, yvel, ftime, opt_subkey, opt_key, mad, vad)
  sigma_n = sigma_n[0]
  elbo_history[step] = -value[0]
  step += 1
...
for i in trange(fine_steps):  
  sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, opt_subkey, opt_key, mad, vad, value = _FUpdate(i, sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, num_f, num_b, num_ind, num_filt, batch_size, k, n, indices, spikefft, yvel, ftime, opt_subkey, opt_key, mad, vad)
  sigma_n = sigma_n[0]
  elbo_history[step] = -value[0]
  step += 1

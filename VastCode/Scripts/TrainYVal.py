...
f_maxt = 1.5 # Filter horizon CHECK THESE
...
print((0.5 * jnp.sin( jnp.pi * (iz_fracs - 0.5)) + 0.5) * (ftime[-1, 0] - ilag) + ilag)
key, *subkeys = random.split(key, 4)
iv_vector = 0.5 * random.normal(subkeys[-3], (num_mach, batch_size, num_ind, 1))
...
# %%
coarse_steps = 1500 # CHECK THESE
mid_steps = 16500
num_savem = 5
eachbatch = int(16500 / num_savem)
fine_steps = 2000

elbo_history = np.zeros(coarse_steps + mid_steps + fine_steps)
# %%
print(PNeg(sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag, num_f, num_b, num_ind, num_filt, batch_size, k, n, numdat, indices, spikefft, yvel, ftime, opt_subkey))
...
savepath = pt.abspath(pt.join(pt.dirname(__file__), '..', 'Data/Hyperparameters/LinearSum/' + str(n) + '_' + str(attemptno) + 'yvel' + '.pkl'))
with open(savepath, 'wb') as f:
    pickle.dump(outdict, f)

# %%
for j in range(num_savem):
  for i in trange(eachbatch): 

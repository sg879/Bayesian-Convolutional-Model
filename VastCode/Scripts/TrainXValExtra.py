...
loadpath = pt.abspath(pt.join(pt.dirname(__file__), '..', 'Data/Hyperparameters/LinearSum/' + str(n) + '_' + str(attemptno) + 'xvel' + '.pkl'))

# %%
with open(loadpath, 'rb') as f:
    xdict = pickle.load(f)

sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag = xdict["Parameters"]
elbo_history = xdict["ELBO History"]
trainind = xdict["Training Sets"]
steparr = xdict["Steps"]
steplab = xdict["Step Labels"]
isigma_f, iell, isigma_n, iz_fracs, iv_vector, il_diag, il_odiag, itrise, itaudiff, ilag = xdict["Initial Parameters"]
num_f, num_b, num_ind, num_filt, num_tbin, num_mach = xdict["Training Parameters"]
f_maxt, time_bin = xdict["Filter Form"]
mad = xdict["mad"]
vad = xdict["vad"]
...
# %%
# Get x velocities
xraw = [data[i].finger_vel.x.to_numpy()[:num_tbin:subsamp].reshape(n + 1, 1) for i in trainind]
...
savepath = pt.abspath(pt.join(pt.dirname(__file__), '..', 'Data/Hyperparameters/LinearSum/' + str(n) + '_' + str(attemptno) + '.' + str(attemptadd) + 'xvel' + '.pkl'))
...
outdict = {"Parameters": (sigma_f, ell, sigma_n, z_fracs, v_vector, l_diag, l_odiag, trise, taudiff, lag),
          ...
          "Steps": steparr.append(extra_steps),
          "Step Labels": steplab.append(steps_label),
